    ...
    for i in range(len(input_list)):
        for j in range(len(input_list[i])):
            ...
            for k in range(len(input_list[i][j])):
                if input_list[i][j][k] > max:
                    max = input_list[i][j][k]
    return max 

def min_2dlist_tuples(input_list):
    min = input_list[0][0][0]
    for i in range(len(input_list)):
        for j in range(len(input_list[i])):
            if input_list[i][j] == None:
                continue
            if not isinstance(input_list[i][j],list):
                if input_list[i][j] < min:
                    min = input_list[i][j]
                continue 
            for k in range(len(input_list[i][j])):
                if input_list[i][j][k] < min:
                    min = input_list[i][j][k]
    return min 
...
class GridWorld:
    def __init__(self):
        ...
        self.gw = Canvas(self.root, width=width+1, height=height+1, borderwidth=0, highlightthickness=0)
        self.gw.pack()
...
def choose_action(state,num_episodes,max_episodes, Qvals): #epsilon is the probability that you act randomly 
    ...
    epsilon = num_episodes/float(max_episodes)*0.05 + (1-num_episodes/float(max_episodes))*1.
